...
const DIACRITICS = defaultDiacriticsRemovalMap.reduce(function (prev: any, cur) {
...
const CLEANFCT = [
    { name: "Trans Majuscule    ", op: function (value: string) { return !value ? value : value.toUpperCase(); } },
    { name: "Suppr diacritiques ", op: function (value: string) { return !value ? value : value.replace(DIACRITICSRE, function (_, p) { return DIACRITICS[p]; }); } },
    { name: "Suppr parenthese   ", op: function (value: string) { return !value ? value : value.replace(/^[^(]*\(/, " ").replace(/\)[^)]*$/, " "); } },
    { name: "Suppr liaisons     ", op: function (value: string) { return !value ? value : value.replace(SUPPRRE, ""); } },
    { name: "Abbrev type voie   ", op: function (value: string) { return !value ? value : value.replace(ABBREVRE, function (_, p) { return ABBREV[p]; }); } },
    { name: "Suppr no alphanum  ", op: function (value: string) { return !value ? value : value.replace(ALPHANUMRE, " "); } }
]
const CLEANWD = [
    { name: "Trans Majuscule    ", op: function (value: string) { return !value ? value : value.toUpperCase(); } },
    { name: "Suppr diacritiques ", op: function (value: string) { return !value ? value : value.replace(DIACRITICSRE, function (_, p) { return DIACRITICS[p]; }); } },
    ...
    { name: "Abbrev type voie   ", op: function (value: string) { return !value ? value : value.replace(ABBREVRE, function (_, p) { return ABBREV[p]; }); } },
    { name: "Suppr no alphanum  ", op: function (value: string) { return !value ? value : value.replace(ALPHANUMRE, " "); } },
    { name: "Suppr exces blancs ", op: function (value: string) { return !value ? value : value.replace(/^ */, "").replace(/ *$/g, "").split(/ +/); } }
]
const CLEANWORDS = [
    { name: "Trans Majuscule    ", op: function (value: string) { return !value ? value : value.toUpperCase(); } },
    { name: "Suppr diacritiques ", op: function (value: string) { return !value ? value : value.replace(DIACRITICSRE, function (_, p) { return DIACRITICS[p]; }); } },
    { name: "Suppr liaisons     ", op: function (value: string) { return !value ? value : value.replace(SUPPRRE, " "); } },
    { name: "Suppr no alphanum  ", op: function (value: string) { return !value ? value : value.replace(ALPHANUMRE, " "); } },
    { name: "Suppr exces blancs ", op: function (value: string) { return !value ? value : value.replace(/^ */, "").replace(/ *$/g, "").replace(/ +/g, " "); } }
]
/**
 * Mapping caractère bit pour le calcul du fuzzy hash
 * ATTENTION : J et G, K et Q, S et Z V et W  sont confondus (pour rester sur 32 bits) 
 */
const FUZZYMAP:{[char:string]: number} = {
    "A" : 0b1,
    "B" : 0b10,
    "C" : 0b100,
    "D" : 0b1000,
...
class Strtools {
    ...
    wordList(value: string): string[] {
        if (!value) return []
        const cleaned = CLEANWD.reduce((value: any, fct) => {
            const next = fct.op(value)
            //console.log(fct.name +" "+next)
            return next
        }, value)
    ...
    cleanWords(value: string): string {
        if (!value) return ""
        const cleaned = CLEANWORDS.reduce(function (value: any, fct) {
            const next = fct.op(value)
            //console.log(fct.name +" "+next)
            return next
        }, value)
    ...
    fuzzyHash(value: string): number {
        ...
        const chars: string[] = cleaned.split('')
    ...
    fuzzyExtend(fuzzyh: number): number[] {
        // calculer les dérivé à 2 caractere pres du code fuzzyh
        const onechardiff = []
